...
router.post('/checkin', upload.single('images'), validatePresence, async (req, res) => {
  try {
    ...
    //   });
    // }
    const schedule = await db.Schedules.findOne({ where: { day: moment().tz('Asia/Jakarta').locale('id').format('dddd') } });
    ...
    const nowDate = moment().tz('Asia/Jakarta').locale('id').format('YYYY-MM-D');
    const tolerant = moment(nowDate+"T"+schedule.clockIn+"+07:00").add(-30, 'minutes').format();
    ...
    if(req.body.coordinates) {
      ...
      if (!isNaN(lat) && !isNaN(lng)) {
        ...
        if (req.body.source.toLowerCase() == 'android') {
          if (lat.toString().split('.')[1].length > 7 || lng.toString().split('.')[1].length > 7) {
            return res.status(400).json({
    ...
    const presence = await db.Presence.findOne({
      where: {
        employeeId, 
        checkIn: {
            [db.Sequelize.Op.gte]: moment().tz('Asia/Jakarta').locale('id').startOf('day'),
            [db.Sequelize.Op.lt]: moment().tz('Asia/Jakarta').locale('id').endOf('day'),
        }
      },
      order: [['checkIn', 'ASC']]
    });
...
router.post('/checkout',upload.single('images'), validatePresence, async (req, res) => {
  try {
    ...
    const nowDate = moment().tz('Asia/Jakarta').locale('id').format('YYYY-MM-D');
    const tolerant = moment(nowDate+"T"+schedule.clockOut+"+07:00").add(+30, 'minutes').format();
